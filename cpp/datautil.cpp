...
// Conference, Days
void DataUtil::prepareTokyo201902() {  // TOKYO = WELKOME DAY in 17cng
    ...
    QString venueAddress;
    venueAddress = "Laboratório Nacional de Engenharia Civil (LNEC)";
    venueAddress.append("\n");
    venueAddress.append("Centro de congressos do LNEC");
    venueAddress.append("\n");
    venueAddress.append("Avenida do Brasil, 101");
    venueAddress.append("\n");
    venueAddress.append("1700-066 Lisboa, ");
    venueAddress.append("Portugal");
    conference->setAddress(venueAddress);
    conference->setMapAddress(" ");
    conference->setTimeZoneName("LONDON");
    conference->setTimeZoneOffsetSeconds(0 * 60 * 60); // +00:00 GMT
    conference->setConferenceFrom(QDate::fromString("2030-05-03", YYYY_MM_DD)); // mudei a data para passar para a frente
    conference->setConferenceTo(QDate::fromString("2030-05-03", YYYY_MM_DD));
    conference->setHashTag("#17cng2020");
    conference->setHomePage("http://17cng2020.lnec.pt");
    QString coordinate;
    coordinate = QString::number(38.754761)+","+QString::number(-9.150340);
    conference->setCoordinate(coordinate);
    conference->setPlaceId("ChIJYe0QMFQyGQ0R7_96Atrgx08");  //see https://developers.google.com/places/place-id
    ...
    Day* day = mDataManager->createDay();
    day->setId(2019021);
    day->setWeekDay(5); // Sunday
    day->setConference(201902);
    day->setConferenceDay(QDate::fromString("2030-05-03", YYYY_MM_DD));  // mudei para passar para depois
    conference->addToDays(day);
    mDataManager->insertDay(day);
//    // tuesday
//    day = mDataManager->createDay();
...
// Conference, Days
void DataUtil::prepareBerlin201901() {   //Berlin tem os dias todos domingo a quarta
    qDebug() << "prepareBerlin201901";
    Conference* conference = mDataManager->createConference();
    conference->setId(201901);
    conference->setConferenceName("17cng2020");
    conference->setConferenceCity("Lisboa");
    QString venueAddress;
    venueAddress = "Centro de congressos do LNEC";
    venueAddress.append("\n");
//    venueAddress.append("Laboratório Nacional de Engenharia Civil (LNEC)");
//    venueAddress.append("\n");
    venueAddress.append("Avenida do Brasil, 101");
    venueAddress.append("\n");
    venueAddress.append("1700-066 Lisboa, ");
    ...
    conference->setTimeZoneName("LONDON");
    conference->setTimeZoneOffsetSeconds(0 * 60 * 60); // +00:00 GMT
    conference->setConferenceFrom(QDate::fromString("2020-05-03", YYYY_MM_DD));
    conference->setConferenceTo(QDate::fromString("2020-05-06", YYYY_MM_DD));
    conference->setHashTag("#17cng2020");
    conference->setHomePage("http://17cng2020.lnec.pt");
    QString coordinate;
    coordinate = QString::number(38.758820)+","+QString::number(-9.141150);
    conference->setCoordinate(coordinate);
    conference->setPlaceId("ChIJYe0QMFQyGQ0R7_96Atrgx08");  //see https://developers.google.com/places/place-id
    ...
    day->setWeekDay(7);
    day->setConferenceDay(QDate::fromString("2020-05-03", YYYY_MM_DD));
    conference->addToDays(day);
    mDataManager->insertDay(day);
    // monday
    day = mDataManager->createDay();
    ...
    day->setConference(201901);
    day->setWeekDay(1);
    day->setConferenceDay(QDate::fromString("2020-05-04", YYYY_MM_DD));
    conference->addToDays(day);
    mDataManager->insertDay(day);
    // tuesday
    day = mDataManager->createDay();
    ...
    day->setConference(201901);
    day->setWeekDay(2);
    day->setConferenceDay(QDate::fromString("2020-05-05", YYYY_MM_DD));
    conference->addToDays(day);
    mDataManager->insertDay(day);
    // wednesday
    day = mDataManager->createDay();
    ...
    day->setConference(201901);
    day->setWeekDay(3);
    day->setConferenceDay(QDate::fromString("2020-05-06", YYYY_MM_DD));
    conference->addToDays(day);
    mDataManager->insertDay(day);
...
void DataUtil::startUpdate()
{
    ...
    emit progressInfo(mProgressInfotext);
    mDataServer->requestSchedule(201901);    // AQUI DEFINE_SE o congresso inicial?
}
...
void DataUtil::continueUpdate()
{
    ...
    // S P E A K E R
    mProgressInfotext = tr("Sync Authors");
    emit progressInfo(mProgressInfotext);
    ...
    if(dataList.empty()) {
        ...
        emit updateFailed(tr("Update failed. No Author received.\nReloading current Data"));
        return;
    }
...
bool DataUtil::updateSessions(const int conferenceId) {
    ...
    if(conferenceId == 201901) {
        ...
        myDay= "2030-05-03";  // tokyo   passei para a frente
        if(allDaysMap.contains(myDay)) {
            serverDayList.append(allDaysMap.value(myDay).toMap());
        } else {
            qDebug() << "Day missed in conference-days from server API " << myDay;
...
void DataUtil::addGenericSessionsBerlin201901() {
    ...
    for (int i = 0; i < conference->days().size(); ++i) {
        ...
        if(day->conferenceDay().toString(YYYY_MM_DD) == "2020-05-03") {
            ...
            mMultiSession.insert(session->sortKey(), session);
            mDataManager->insertSession(session);
        } else if(day->conferenceDay().toString(YYYY_MM_DD) == "2020-05-05") {
            // CONF DAY
            // REGISTRATION
            session = mDataManager->createSession();
            lastGenericSession --;
...
void DataUtil::addGenericSessionsTokyo201902() {
    ...
    for (int i = 0; i < conference->days().size(); ++i) {
        Day* day = conference->days().at(i);
        if(day->conferenceDay().toString(YYYY_MM_DD) == "2030-05-03") {
            // REGISTRATION
            session = mDataManager->createSession();
            lastGenericSession --;
            session->setSessionId(lastGenericSession);
            ...
            session->setIsGenericScheduleSession(true);
            session->setIsRegistration(true);
            session->setStartTime(QTime::fromString("08:40", HH_MM));
            session->setEndTime(QTime::fromString("09:25", HH_MM));
            ...
            session->setIsGenericScheduleSession(true);
            session->setIsBreak(true);
            session->setStartTime(QTime::fromString("14:00", HH_MM));
            session->setEndTime(QTime::fromString("14:15", HH_MM));
            session->setMinutes(15);
            session->setConference(conferenceId);
            session->setSessionDay(day->id());
            session->setSortKey(day->conferenceDay().toString(YYYY_MM_DD)+session->startTime().toString(HH_MM));
            mMultiSession.insert(session->sortKey(), session);
            ...
            session->setIsGenericScheduleSession(true);
            session->setIsBreak(true);
            session->setStartTime(QTime::fromString("16:45", HH_MM));
            session->setEndTime(QTime::fromString("17:00", HH_MM));
            session->setMinutes(15);
            session->setConference(conferenceId);
            session->setSessionDay(day->id());
            session->setSortKey(day->conferenceDay().toString(YYYY_MM_DD)+session->startTime().toString(HH_MM));
...
void DataUtil::finishUpdate() {
    ...
    while (speakerImagesIterator.hasNext()) {
        ...
        if(!mDataManager->findSpeakerImageBySpeakerId(speakerImage->speakerId())) {
            ...
            qDebug() << "detected UPDATED Speaker Image: " << speakerImage->speakerId();
        }
    }
...
Conference* DataUtil::currentConference() {
    ...
    if(!mCurrentConference) {
        ...
        mCurrentConference = static_cast<Conference*>( mDataManager->allConference().first());  //RIC Passar para last() para mudar a ordem das conferencias
        qDebug() << "Current Conference: " << mCurrentConference->conferenceCity();
    }
    return mCurrentConference;
}
